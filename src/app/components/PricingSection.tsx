...
const PricingSection = () => {
  const [selectedPlanIndex, setSelectedPlanIndex] = useState<number | null>(1); // Default selected to 'Pro'
          ...
          {plans.map((plan, index) => {
            ...
            return (
              <div
                ...
                className={`relative cursor-pointer bg-white rounded-2xl shadow-lg transition-all duration-300 p-8
                ${isSelected ? 'ring-2 ring-[#8A3FFC] scale-105' : 'hover:shadow-xl'}`}
              >
                {isSelected && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <span className="bg-gradient-to-r from-[#8A3FFC] to-[#4F82FF] text-white px-4 py-2 rounded-full text-sm font-semibold">
